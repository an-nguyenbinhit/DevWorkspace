...
`));let e=[],n=t.split(/(\n|\r\n)/);n[n.length-1]||n.pop();for(let i=0;i<n.length;i++){let r=n[i];i%2&&!o.newlineIsToken?e[e.length-1]+=r:e.push(r)}return e}var on=class t{compute(o,e,n,i=new Set){let r=this.applyIgnoreOptions(o,n),s=this.applyIgnoreOptions(e,n),l=ti(r,s,{ignoreWhitespace:n.ignoreWhitespace}),a=this.pairLines(l),h=this.buildHunks(a,n.contextLines,i),g=[],y=[],m=[],C=[],z=0,Y=0,de=0,le=0;for(let te of h)for(let $ of te){if($.type==="fold"){let at={lineNumber:null,type:"fold",raw:"",tokens:null,highlightedHtml:"",foldedCount:$.foldedCount,hunkIndex:$.hunkIndex},Lt=ce({},at);g.push(at),y.push(Lt),m.push({left:at,right:Lt,index:m.length}),C.push(at);continue}let _e=$.leftText??"",Qe=$.rightText??"",et=null,tt=null;if($.type==="modified"&&(n.wordDiff||n.charDiff)){let{left:at,right:Lt}=this.computeWordDiff(_e,Qe,n.charDiff);et=at,tt=Lt}let we={lineNumber:$.leftLineNum,type:$.type==="added"?"unchanged":$.type,raw:this.escapeHtml(_e),tokens:et,highlightedHtml:this.escapeHtml(_e)},Se={lineNumber:$.rightLineNum,type:$.type==="removed"?"unchanged":$.type,raw:this.escapeHtml(Qe),tokens:tt,highlightedHtml:this.escapeHtml(Qe)};$.type==="added"?(we.type="unchanged",we.lineNumber=null,we.raw="",we.highlightedHtml="",Se.type="added",z++):$.type==="removed"?(we.type="removed",Se.type="unchanged",Se.lineNumber=null,Se.raw="",Se.highlightedHtml="",Y++):$.type==="modified"?(we.type="modified",Se.type="modified",de++):(we.type="unchanged",Se.type="unchanged",le++),g.push(we),y.push(Se),m.push({left:we,right:Se,index:m.length}),$.type==="added"?C.push(Se):$.type==="removed"?C.push(we):$.type==="modified"?(C.push(we),C.push(Se)):C.push(we)}let M=z+Y+de+le,X=M>0?Math.round(le/M*100):100;return{flatLeft:g,flatRight:y,sideBySideRows:m,inlineRows:C,totalAdded:z,totalRemoved:Y,totalModified:de,totalUnchanged:le,similarityPercent:X}}applyIgnoreOptions(o,e){let n=o.split(/\r?\n|\r/);return e.ignoreBlankLines&&(n=n.filter(i=>i.trim().length>0)),e.trimLines?n=n.map(i=>i.trim()):e.ignoreWhitespace&&(n=n.map(i=>i.trimEnd())),e.ignoreComments&&(n=n.filter(i=>!this.isCommentLine(i))),n.join(`
